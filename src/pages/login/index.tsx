...
import Logo from "../../../public/images/logo-header.jpeg";
...
import { Form, Validators, useForm } from "@/lib/components/form";
import { TextField } from "@/components/form/TextField";
import { Input } from "@material-tailwind/react";
import { MIN_LENGHT_PASSWORD } from "@/constants/common";
...
const Login = () => {
	...
	const fLogin = useForm<iLogin>({ email: {value : null}, password: {value : null} });
	...
	const handleLogin = async () => {
		setIsLoading(true);
		const data: any = await login(fLogin.controls.email.value, fLogin.controls.password.value)
		if (data.user) router.push(Paths.dashboard, undefined, { shallow: true });
		else {
			setIsLoading(false);
			setError(true);
		}
		...
		<>
			<div className="m-autobg-gray-50 h-screen flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
				<div className="sm:mx-auto sm:w-full sm:max-w-md">
					<div>
						<Image width={72} height={72} className="mx-auto h-12 w-auto" src={Logo} alt="full-data-pro-logo" />
					</div>
				...
				<div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
					<div className="bg-white py-8 px-4 sm:rounded-lg sm:px-10">
						<Form className="space-y-6" group={fLogin} onSubmit={handleLogin}>
							<TextField
								name="email"
								control={fLogin.controls.email}
								label={t(`global.email`) as any}
							...
							<TextField
								name="password"
								type="password"
								control={fLogin.controls.password}
								label={t(`global.password`) as any}
